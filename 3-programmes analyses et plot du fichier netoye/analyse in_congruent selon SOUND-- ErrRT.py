...
classeur = "Sujet_N_MutilAnalysis-nettoyé1.xlsx"
...
def etude_stimuli(L,R,n):
    ...
    for j in range(len(L)):
        ...
        for k in range(1,136):
            if int(L[j][k,2]) == R:
                if int(L[j][k,3]) == n:
                    if float(L[j][k,6]) != 0: 
                        TR.append(float(float(L[j][k,6])))
                    if np.isnan(float(L[j][k,7])) == False:
                        Errh.append(abs(float(L[j][k,11])))
                        Err3D.append(abs(float(L[j][k,7])))
    ...
    return TR,Errh,Err3D
                    


    
                        
#Son de 1 à 9 dans 3 scènes
Err_3D = []
Err_h = []
for i in range (1,10):
...
#Son de 19 à 21 dans 3 scènes

Err_3D7 = []
for i in range (19,22):
    TR,Errh,Err3D = etude_stimuli(listeFeuilles,8,i)
    for j in range(len(Err3D)):
        Err_3D7.append(Err3D[j])

Err_3D8 = []
for i in range (19,22):
    TR,Errh,Err3D = etude_stimuli(listeFeuilles,10,i)
    for j in range(len(Err3D)):
        Err_3D8.append(Err3D[j])

Err_3D9 = []
for i in range (19,22):
    TR,Errh,Err3D = etude_stimuli(listeFeuilles,0,i)
    for j in range(len(Err3D)):
        Err_3D9.append(Err3D[j])


### BoxPlot de congruent/incongruent dans plage_8 ###

plt.figure(1)
boxplotElements = plt.boxplot([Err_3D,Err_3D2,Err_3D3], sym = 'b.', whis = 1.5,
                                 widths = [1,1,1], positions = [1,2,3],
                                 patch_artist = True)
plt.gca().xaxis.set_ticklabels(['Plage', 'Eglise','Gris'])
...
for element in boxplotElements['caps']:
    element.set_color('red')
plt.title('Son de la plage dans plage/eglise/gris')
...
plt.figure(2)
boxplotElements = plt.boxplot([Err_3D4,Err_3D5,Err_3D6], sym = 'b.', whis = 1.5,
                                 widths = [1,1,1], positions = [1,2,3],
                                 patch_artist = True)
plt.gca().xaxis.set_ticklabels(['Plage', 'Eglise','Gris'])
for element in boxplotElements['medians']:
...
for element in boxplotElements['caps']:
    element.set_color('red')
plt.title("Son de l'église dans plage/eglise/gris")
plt.ylabel(' Erreur 3D (m) ')
plt.grid(True)
...
plt.figure(3)
boxplotElements = plt.boxplot([Err_3D7,Err_3D8,Err_3D9], sym = 'b.', whis = 1.5,
                                 widths = [1,1,1], positions = [1,2,3],
                                 patch_artist = True)
plt.gca().xaxis.set_ticklabels(['Plage', 'Eglise','Gris'])
for element in boxplotElements['medians']:
...
for element in boxplotElements['caps']:
    element.set_color('red')
plt.title('Son de neutre dans plage/eglise/gris')
plt.ylabel(' Erreur 3D (m) ')
plt.grid(True)
